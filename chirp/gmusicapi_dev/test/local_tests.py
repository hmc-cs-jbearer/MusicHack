...
from gmusicapi_dev.clients import Webclient, Musicmanager
from gmusicapi_dev.exceptions import AlreadyLoggedIn
from gmusicapi_dev.protocol.shared import authtypes
from gmusicapi_dev.protocol import mobileclient
from gmusicapi_dev.utils import utils, jsarray
...
def create_clients():
    clients = []
    for name in names:
        cls = getattr(gmusicapi_dev.clients, name)
        c = cls()
...
def create_sessions():
    sessions = []
    for name in names:
        cls = getattr(gmusicapi_dev.session, name)
        s = cls()
