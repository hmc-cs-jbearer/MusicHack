...
from gmusicapi_dev import __version__
from gmusicapi_dev.appdirs import my_appdirs
from gmusicapi_dev.exceptions import CallFailure, GmusicapiWarning, NotSubscribed
...
log_filepath = os.path.join(my_appdirs.user_log_dir, 'gmusicapi_dev.log')
                ...
                try:
                    if 'self' in frame.f_locals:
                        ...
                        if ((f_self is not None and
                             f_self.__module__.startswith('gmusicapi_dev.clients') and
...
def configure_debug_log_handlers(logger):
    """Warnings and above to stderr, below to gmusicapi_dev.log when possible.
...
def retry(retry_exception=None, tries=5, delay=2, backoff=2, logger=None):
    ...
    :param logger: logger to use. If None, use 'gmusicapi_dev.utils' logger
    ...
    if logger is None:
        logger = logging.getLogger('gmusicapi_dev.utils')
