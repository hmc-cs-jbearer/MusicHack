...
class _Base(with_metaclass(utils.DocstringInheritMeta, object)):
    ...
    def __init__(self, logger_basename, debug_logging, validate, verify_ssl):
        ...
        :param debug_logging: each Client has a ``logger`` member.
          The logger is named ``gmusicapi_dev.<client class><client number>`` and
          will propogate to the ``gmusicapi_dev`` root logger.
        ...
        logger_name = "gmusicapi_dev.%s%s" % (logger_basename,
                                          _Base.num_clients)
        self._cache = {}
