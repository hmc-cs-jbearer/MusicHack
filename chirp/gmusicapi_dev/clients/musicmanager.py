...
import gmusicapi_dev
from gmusicapi_dev.clients.shared import _Base
from gmusicapi_dev.appdirs import my_appdirs
from gmusicapi_dev.exceptions import CallFailure, NotLoggedIn
from gmusicapi_dev.protocol import musicmanager, upload_pb2, locker_pb2
from gmusicapi_dev.utils import utils
from gmusicapi_dev import session
...
class Musicmanager(_Base):
    ...
    For most authors and users of gmusicapi_dev scripts,
    ...
    def perform_oauth(storage_filepath=OAUTH_FILEPATH, open_browser=False):
        ...
        :param storage_filepath: a filepath to write the credentials to,
          ...
          ``user_data_dir`` is used by default. Users can run::
              ...
              import gmusicapi_dev.clients
              print gmusicapi_dev.clients.OAUTH_FILEPATH
    ...
    def login(self, oauth_credentials=OAUTH_FILEPATH,
        ...
        :param uploader_name: human-readable non-unique id; default is
          ``"<hostname> (gmusicapi_dev-{version})"``.
    ...
    def _perform_upauth(self, uploader_id, uploader_name):
        ...
        if uploader_name is None:
            uploader_name = gethostname() + u" (gmusicapi_dev-%s)" % gmusicapi_dev.__version__
    ...
    def download_song(self, song_id):
        ...
        <gmusicapi_dev.clients.Webclient.get_song_download_info>`,
    ...
    def upload(self, filepaths, transcode_quality='320k', enable_matching=False):
        ...
        :param enable_matching: if ``True``, attempt to use `scan and match
          ...
          **WARNING**: currently, mismatched songs can *not* be fixed with the 'Fix Incorrect Match'
          ...
          <gmusicapi_dev.clients.Webclient.report_incorrect_match>`.
          They would have to be deleted and reuploaded with matching disabled
          (or with the Music Manager).
          Fixing matches from gmusicapi_dev may be supported in a future release; see issue `#89
          <https://github.com/simon-weber/gmusicapi_dev/issues/89>`__.
